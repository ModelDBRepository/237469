...
tstop = 11000.0
currCoeff = 0.9 # use the threshold current I*0.9 for inducing the first spike
ITERS = 20
...
""")
           ...
           print "nextCoeffs="+str(nextCoeffs)+", "+str(nSpikes_total)+" spikes, iter="+str(iter)
           if iterI==0:
...
"""+mutvar+""" = """+str(newVal[whichDefVal[iupdated]])+"""
    ...
    gCoeffsThisComb.append(gCoeffsThisIter[:])
    picklelist = [theseCoeffsAllAll,gCoeffsThisComb,PPIdts,MT]
    file = open('ppispthrcoeff300_relthr_scaledonly_cs'+str(icell)+'_comb_iiter'+str(iiter)+'.sav', 'w')
